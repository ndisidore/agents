...
2. a **durable, pollable last-known state** (`getPersistedServerState`, level-triggered) that a _hibernating_ consumer reads on its own wake to reconcile what it missed, then resumes subscribing.
...
- **Stays in core (`mcp/client.ts`, `index.ts`)** — a general-purpose slice; the snapshot is the load-bearing piece for the consumer-side story:
  ...
  - **Durable, pollable state (the poll-on-wake enabler):** a `cf_agents_mcp_server_state` sibling table holding `{ state, error }` per server (state is `auth_url`-aware → `AUTHENTICATING`), written by the chokepoint and read via `getPersistedServerState()` / `listPersistedServerStates()`. Sibling table (lazily created, no schema-version bump) so the snapshot survives `cf_agents_mcp_servers`' `INSERT OR REPLACE` rewrites. The snapshot is **level-triggered** (consumers apply the latest state, idempotently); removal simply **deletes** the row. There is no per-server version cursor — see [Why no version cursor](#why-no-version-cursor).
...
5. **Durable state storage:** a `cf_agents_mcp_server_state` sibling table (not columns on `cf_agents_mcp_servers`) — avoids the `INSERT OR REPLACE` rewrite clobbering the snapshot, and a fleet-wide schema migration. The snapshot is **level-triggered**: consumers apply the latest `{ state, error }`, idempotently. Removal simply **deletes** the row (a later re-add of the same id starts clean and the next notify repopulates it from live state); a `migrateServerId` deletes the old row and the post-migration notify repopulates the new id. There are no tombstones, no orphan-prune, and no version cursor — see [Why no version cursor](#why-no-version-cursor).
...
- **Awake consumer** subscribes to the owner's live signal (`onServerStateChanged`, payload) — app wiring (e.g. WebSocket/RPC).
- **Hibernating consumer** reconciles on its _own_ wake: in `onStart`, read the owner's `getPersistedServerState(serverId)` (app-level cross-DO RPC) and apply the latest state (level-triggered, idempotent) — then resume subscribing.
...
| **2** | Durable `{ state }` snapshot + `getPersistedServerState` — the level-triggered poll-on-wake enabler             | shipped |
| **3** | Ongoing durable per-server subscription — retires the app re-arm for banner-style "track this server over time" | future  |

Phase 3 is intentionally deferred: an ongoing subscription is only truly durable when built on the persisted snapshot (you cannot replay edge transitions you never recorded), so Phase 2 is its prerequisite. If Phase 3 ships, it would reintroduce an ordering cursor (a counter or `updated_at`) at that time, to drive a true level-triggered _subscription_ where applying out-of-order deltas matters. Until then, "ongoing" is achieved by the app re-arming a one-shot watch after each settle, and the snapshot stays a cursorless latest-wins value.
...
**Why not add `REMOVED` to the `MCPConnectionState` enum?** It was considered and rejected. Mechanically it is low-risk (the one core `switch` on connection state has a `default`, there are no exhaustive `Record<MCPConnectionState, …>` maps or `assertNever` guards, and example `=== "ready"` checks degrade gracefully). But it models the wrong axis: `MCPConnectionState` is the state of a _connection_, whereas "removed" is the absence of a _server_. A new enum member would (a) never appear on any live `MCPConnection.connectionState`; (b) duplicate the purpose-built `onServerRemoved` event; (c) collide with the snapshot's existing `null` (no-resolvable-state) value as a second encoding of "gone"; (d) make `watchMcpServerSettled({ state: "removed" })` type-legal but nonsensical (removal is handled by the cancel-on-removal path, not target matching); and (e) widen a stable, exported enum, silently making any _external_ exhaustive consumer incomplete. Option Y gets the honesty without any of that.
...
- **Id-migration safety:** `migrateServerId` now fires `onServerIdMigrated { oldId, newId }` (after the rename, before the post-migration state notification). The mixin re-targets live serverId-keyed intents `oldId → newId` so a `{ serverId }` watch follows the rename instead of stranding until its deadline (or forever); the subsequent `onServerStateChanged(newId)` then settles any re-targeted intent already in a target state. URL-targeted intents are unaffected (the url is unchanged by a rename). The snapshot side is a plain row delete of `oldId`; the post-migration notify repopulates `newId` from live state.

## Why no version cursor

An earlier draft attached a monotonic per-server `version` to both the live `onServerStateChanged` payload and the durable snapshot, so a cross-DO consumer could dedupe by `if (incoming.version > applied.version)` — ordered dedupe across the awake-push vs poll-on-wake race. We dropped it.

The value it bought was narrow: it strictly orders the one race where a late-arriving _older_ push could land after a _newer_ state the consumer already polled. For the canonical consumer (a readiness banner), losing that race is cosmetic and **self-correcting** — the next push or poll reconciles to the latest state.

The mechanics it cost were not narrow, and they were **always-on for every MCP agent**, not just settlement users. Keeping a counter monotonic across the server lifecycle required: removal **tombstones** (keep the row, clear state, bump `version`) instead of a plain delete; an **orphan-prune + TTL** to bound the tombstone growth those introduced, run on every wake; and a **`migrateServerId` snapshot-merge** carrying `max(old, new) + 1` so the cursor couldn't regress onto a recycled id. That is ~110 lines of durable-table machinery whose sole purpose was protecting a counter that, when its one race is lost, fixes itself on the next poll.

Decision: the snapshot is **level-triggered** — `{ state, error }`, applied idempotently, removal deletes the row. Ordered dedupe is a nice-to-have we judged not worth its disproportionate, always-on machinery. If Phase 3 ships a true level-triggered _subscription_ (where out-of-order delta application would actually matter), it can reintroduce a cursor then — and the retained `updated_at` column is already a free one.
