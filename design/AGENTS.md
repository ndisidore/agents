...
| `rfc-durable-mcp-settlement.md`         | RFC        | Durable owner-DO MCP settlement (experimental mixin) + payload-bearing `onServerStateChanged` live event and `onServerRemoved`; cross-DO fan-out (poll-on-wake via owner Agent state) is app-level. |
